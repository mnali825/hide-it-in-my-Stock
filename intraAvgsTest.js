...
function percentChange(err, res, body) {
  if (!err && res.statusCode === 200) {
    ...
    for (i = 0; i < dailyOpen.length-100; i++) {
      var mostRecent = dailyOpen.length-100-i;
      var last = dailyOpen.length-i;
      var topen = new timeseries.main(timeseries.adapter.fromArray(dailyOpen.slice(dailyOpen.length-100-i,dailyOpen.length-i)));
      ...
      var actualChange = dailyOpen[mostRecent-1]-dailyOpen[mostRecent];
...
// schedule.scheduleJob(rule, function(){
  request('https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=TSLA&interval=1min&outputsize=full&apikey=RVX8WRJR2THGLQ0Q', percentChange)
// });